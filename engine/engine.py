...
def train(train_loader, model, optimizer, scheduler, scaler, epoch, args):
    ...
    for i, (image, text, target) in enumerate(train_loader):
        ...
        # flops,params = profile(model,inputs=(image, text, target))
    ...
    for img, param in tbar:
        ...
        if args.visualize:
            ...
            cv2.imwrite(filename=os.path.join(args.vis_dir, mask_name),
                        img=mask)
            cv2.imwrite(filename=os.path.join(args.vis_ori_dir, img_name),
                        img=param['ori_img'][0].cpu().numpy())
            cv2.imwrite(filename=os.path.join(args.vis_ori_dir, mask_name),
                        img= vis(param['ori_img'][0].cpu().numpy(), mask)
)
        ...
        for sent in param['sents']:
            ...
            if args.visualize == True:
                ...
                feature_name = '{}-{}'.format(seg_id, sent_name)
                pred = model(img, text, img_name = feature_name)
            else:
                pred = model(img, text)
            ...
            if args.visualize:
                ...
                cv2.imwrite(filename=os.path.join(args.vis_dir, pred_name),
                            img=pred)
                img_ori_vis = vis(param['ori_img'][0].cpu().numpy(), pred)
                cv2.imwrite(filename=os.path.join(args.vis_ori_dir, pred_name),
                            img=img_ori_vis)
